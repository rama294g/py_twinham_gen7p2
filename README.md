...
> [!WARNING]
> 本ソフトウェアはモーターを駆動します。現状の実装では出力操作スイッチ（左側ユニットは GP6、右側ユニットは GP7）を離した後も出力が約 0.5 秒かけて減衰し、モータードライバーの `nSLEEP` はセンサー異常、メニュー表示、BLE 停止指令などの条件がない限り High のままです。また、起動後のゼロ点設定処理は実装されていません。車椅子へ搭載する前に車輪を浮かせ、低出力で配線、回転方向、姿勢角、PWM 範囲およびスイッチ動作を確認してください。独立した非常停止手段を必ず用意してください。

## 現在の主な機能

- BNO055 の加速度・ジャイロ値を組み合わせた車体角度の推定
- 推定角度を正負それぞれの設定範囲で線形変換する CW / CCW PWM 制御
- ユニットに応じたスイッチ入力（左側は GP6、右側は GP7）によるモーター出力のランプアップ／ランプダウン
...
算出角度 `angle` は、加速度から求めた観測角とジャイロを Kalman 相当の補正および 2 段 LPF に通し、`NEUTRAL` を差し引いた値です。最終角度は `-90` ～ `90` 度に制限されます。正の角度は `0` ～ `ANG_MAX` を `0` ～ `PWM_MAX` に、負の角度は `ANG_MIN` ～ `0` を `PWM_MIN` ～ `0` に線形変換します。この結果が 50% を中心とする CW / CCW の差動指令値になり、正の指令では CCW 側、負の指令では CW 側のデューティーが相対的に大きくなります。

左側ユニットでは GP6、右側ユニットでは GP7 を押すと `switch_gain` が制御周期ごとに `0.04` ずつ増え、約 0.5 秒で 1.0 に達します。離した場合も同じ割合で 0 まで減少します。スイッチの押下は姿勢のゼロ点を設定するものではありません。基準姿勢は `NEUTRAL` で調整してください。

### PWM 計算

`current_pwm_command` に `switch_gain` を掛けた差動指令値を `p`（`-100` ～ `100`）とすると、`motor_cwccw(p)` が設定する各 PWM デューティーは次のとおりです。
...
したがって、例えば `p = 20` では CW 40%・CCW 60%、`p = -20` では CW 60%・CCW 40% です。ただし制御処理は、計算した `p` がちょうど 0 の場合には `motor_cwccw(0)`（計算上は CW 50%・CCW 50%）を呼ばず、`motor_stop()` によって CW 0%・CCW 0% を出力します。このため、角度由来の指令値が 0 なら `switch_gain` が 0 より大きくても出力は 0%・0% です。また、スイッチを離して `switch_gain` が 0 になった場合も `p = 0` となり、同様に 0%・0% へ停止します。
...
`current_pwm_command` は角度から算出したランプ適用前の**差動指令値**であり、CW または CCW の実際の PWM デューティーそのものではありません。実際には、この値に `switch_gain` を掛けた `p` を 50% 中心の差動値として、上記の式で CW / CCW の各デューティーへ変換します（`p = 0` の停止分岐を除く）。
...
2. 左側ユニットでは GP6、右側ユニットでは GP7 の押下／解放時に、約 0.5 秒の出力ランプが意図どおり動作すること
3. `NEUTRAL` と実際の基準姿勢が一致すること
4. 車体の傾きと CW / CCW の回転方向が一致すること
5. 正負それぞれの角度範囲と差動指令値の上限が意図どおりであり、指令値から算出した CW / CCW デューティーが 50% を中心に逆方向へ変化すること
6. 角度由来の指令値が 0 の場合（`switch_gain` が有効な場合を含む）と、スイッチ解放後に `switch_gain` が 0 になった場合の両方で、CW・CCW が 0%・0%へ停止すること
7. LCD または BLE で保存した設定が再起動後も保持されること
8. BNO055 の切断後、監視処理によってモーターが停止すること
