...
> [!WARNING]
> 本ソフトウェアはモーターを駆動します。GP6 を離した後も製品要件により出力を約 0.5 秒かけて減衰させ、そのランプダウン中はモータードライバーの `nSLEEP` を High に保ちます。`switch_gain` と実デューティーがともにゼロになった制御周期で `nSLEEP` を Low にします。センサー異常、メニュー表示、BLE 停止指令はランプを行わず、直ちに PWM をゼロ、`nSLEEP` を Low にします。また、起動後のゼロ点設定処理は実装されていません。車椅子へ搭載する前に車輪を浮かせ、低出力で配線、回転方向、姿勢角、PWM 範囲およびスイッチ動作を確認してください。独立した非常停止手段を必ず用意してください。
...
GP6 を押すと `switch_gain` が制御周期ごとに `0.04` ずつ増え、約 0.5 秒で指令値の 100% に達します。離した場合も同じ割合で 0 まで減少します。押下中と解放後のランプダウン中だけ `nSLEEP` は High になり、`switch_gain` と実デューティーがゼロになった直後に Low へ遷移します。GP6 の押下は姿勢のゼロ点を設定するものではありません。基準姿勢は `NEUTRAL` で調整してください。
...
```text
DATA,<angle>,<joystick>,<switch_pressed>,<current_pwm_command>,<AWAKE|SLEEP>,<OK|NG>
```

`current_pwm_command` は角度から算出したランプ適用前の値です。実際のデューティーは、この値に `switch_gain` を掛けた値になります。MOTOR 状態の `AWAKE` / `SLEEP` はそれぞれ物理的な `nSLEEP` の High / Low を表し、GP6 解放後のランプ中は `AWAKE`、ランプ完了直後は `SLEEP` です。
...
- これらの即時停止条件では途中の GP6 ランプも消去するため、条件解除後はゼロからランプアップします。
...
├── motor_control.py        # GP6 ランプ、PWM、nSLEEP の状態遷移
...
2. GP6 の押下／解放時に約 0.5 秒の出力ランプが動作し、解放ランプ完了と同時に `nSLEEP` が Low になること
